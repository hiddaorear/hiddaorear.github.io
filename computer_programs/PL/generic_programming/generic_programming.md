...
同时，左右界之差是这个系列的长度，还有一个特性：如果我们需要处理一个系列的多个子序列，在1和4方案里，子序列将会是相邻的，上一个子序列的下界，是下一个子序列的上界。
...
从区间的形式分类，可以是双界区间（bounded range），用两个迭代器封闭指向区间的开头和刚刚越过结尾的位置；还可以是计数区间（counted range），用迭代器指向区间开始位置，用n表示区间包含元素的个数。
...
异类字典：一个按照键值对来保存与索引数据的容器。“异类”是指容器中存储的值类型可以不同。如：可以在容器中保存double类型对象，还可以保存`std::string`类型对象。容器的索引对象的键，在编译器指定，基于键的索引工作，也在编译器完成。
