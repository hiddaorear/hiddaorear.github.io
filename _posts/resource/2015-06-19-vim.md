...
`.`这个mark代表最后一次修改的地方，所以``.`可以跳到最后一次修改的地方，`'.`跳转到最后修改的那一行。
而`g;`与`g,`则可以在整个changelist里面来回跳转。
`C-o`与`C-i`这是在jumplist里面来回跳转。


### -20. gd查找
相比`/`或`?`，gd更加快捷。搜索光标所在的单词，并定位到文本中此单词所在的第一地方。

### -19. 缩进

- 从第m行起，到第n行，向右缩进一次：输入`:m,n>`等价`:m>(n-m+1)`
- 从第m行起共n行向右缩进一次：`:m>n`等价`m<(n-m+1)`
向左缩进同上。


### -18. 折叠

`set foldmethod=indent`，根据代码缩进自动折叠。
...
- 折叠文件所有代码：zM
- 取消折叠文件所有折叠：zR
- 第M行到N行折叠：`:m,nfoldopen`，关闭折叠：`:m,nfoldclose`
- 在折叠中上下移动：`[z]z`（在打开的折叠中），`zj,zk`在关闭的折叠中

保存折叠：在离开的时候：`mkview`，在进来的时候：`loadview`
...
### -17. 全局光标移动（重要技巧）
    `C+O`向后回退光标移动；
    `C+I`向前追赶光标移动；
    可以在tab页和windows中向前向后trace光标，非常方便；
...
### -14. 多文件替换
主意提前做好备份，update之后直接修改硬盘。

```shell
:args dir/*.js  # dir为目录
:argdo %s/hate_word/love_word/gc | update
...
`:substituent`，将制定的字符，替换为其他字符，通常使用其缩写`:s`
...
####  制定范围
在可视模式下，选择之后，`:`进入命令模式，利用s命令在选中范围中进行文本替换。

|Command|Function|
|:-----|:--------|
|`%s`    |整个文件|
|`num1, num2s`|num1至num2行|
|`.,$s`  |当前行至文件结尾|
|`.,.+nums`|后修num+1行内进行替换|
...
`m{mark}`选定字母标记当前光标所在位置，`[a-zA-Z]`，小写字母只能在当前缓冲区局部可见，而大写全局可见。
...
[Vim(gvim)实现多文件的查找和替换](http://www.vimer.cn/2009/10/vimgvim%E5%AE%9E%E7%8E%B0%E5%A4%9A%E6%96%87%E4%BB%B6%E7%9A%84%E6%9F%A5%E6%89%BE%E5%92%8C%E6%9B%BF%E6%8D%A2.html)
