...
标准中规定回调函数抛出错误，此时promise实例状态变为reject，此错误会被下一个catch方法指定的回调函数捕获。但是，jQuery的Deferred对象此时不会改变状态，也不会触发回调函数，一般情况下此错误会被window.onerror捕获。即，deferred对象，必须使用reject方法来改变状态。
