...
(function($) {
	$.fn.charCount = function(options) {
		...
		var defaults = {
			...
			container:"",
			counterText: '',
			isChinese:false //一个中文要不要按2个字符算，默认是false，即不按2个字符算，若传true,则按2个字符算
		};
		var $par;
		...
		function calculate(obj) {
			...
			if(available<0) {
				available = -available;
				$par.html("超出<b style='color:red;'>"+ available +"</b>字"); //超出的样式
				
			} else {
				$par.html("您还可以输入<b>"+available+"</b>个字符"); //正常时候
			}
		};
		...
		function validateMaxLength(obj) {
			...
			max.keyup( function() {
				var thisLen=$(this).val().length;
				if (len > 0 && len < thisLen) {
					this.value = $(this).val().substring(0, len);
				}
			})
			max.change( function() {
				var thisLen=$(this).val().length;
			...
			max.on('paste', function() {
				window.setTimeout(function(){max.trigger('keyup');}, 5)
			})
			max.on('cut', function() {
				window.setTimeout(function(){max.trigger('keyup');}, 5)
			})
