...
git commit -a -m --verbose 'devel complete'   # many times
...
git的要义在与branch与merge，即内容的隔离与合并，一言以蔽之：分合而已。其中合并是关键，因为拆分出一个分支，容易，而合并两个分支，对出现的冲突的处理是关键。以上版本处理最为重要的一步，是使用merge，merge能达到代码合并的结果，但同时也带来了分支的噪声。

git使用三向合并，即：两个文件合并，会参照第三个文件。第三个文件是一个处理冲突的标准，大大减少冲突的出现的概率。git合并分支是，先回溯每个分支的历史，找到分支共同的祖先，以此为基础，然后合并。

Linus Torvalds被称为速度怪物(speed freak)，其追求速度到了极致，git历史回溯速度非常之快，以致于分支的合并是弹指一瞬的事儿，所以git里面分支的处理代价极小，大家也就都喜欢用分支来管理代码，有feature就开一个新分支。

所以主干分支，保持线性。开发分支，酌情处理。

### rebase VS merge
- 把一个分支整合到另一个分支的办法有两种：`merge && rebase`。
- `merge` 把俩分支最新快照，以及俩者的共同祖先，进行三方合并，并产生新的commit。
- `rebase`把其一个分支的变化parch在另一个分支的最新commit上重新打一遍，即在把一个分支中的改变移动到另一个分支中重放。
- 俩者整合之后结果无区别，但rebase能产生一个更为整洁的历史。仿佛所有的修改在一条线上进行，尽管其原本是并行发生。
- 一旦分支中的提交发布到公共仓库master，不要对该分支进行rebase操作，会导致master中有一条以上的历史记录。
...
git checkout -b devel
git add .               # many times
git commit -m --verbose 'devel'   # many times
git checkout master     
...
git checkout -b devel
git add .               # many times
git commit --verbose -m 'devel'   # many times
...
cat: /home/code/test/.git/rebase-merge/stopped-sha: No such file or directory
...
````
 ...
 另外，由于有rebase操作，与远程分支不一定能兼容，有可能要强推。
...
所以命名用：`release_*, hotfix_*, feature_*`，release_1.0可以如此命名，这样可以带上版本号，当有新的版本的时候，切换到新的版本。其他feature与hotfix，也可如此。


#### `git merge --no-ff branch_feature`
`--no-ff`参数总会会是merge产生一个提交，即使合并可以快速完成。这个技巧，可以避免feature_branch的历史信息混在主分支之后。如果混在之后，则查找的时候，我们不得不查看每一次提交的log。

### tag

`git tag -a <tag_name> -m 'msg'`其作用是给从commit起一个方便的名字。分支处理结束之后，可以打上tag。
...
git diff --staged  # stage与HEAD的差异
git diff HEAD      # 工作去与上次提交的差异
...
## Undo
 任何已commit的文件都可以恢复，即使分支已删除，或用`--amend`重写过，都可以恢复。如果没有，就可能失去数据，仅仅限于没有提交过的，对git来说就像未存在过一样。

![起风了](/../../assets/img/tech/2015/git-checkout-reset.png)
...
````
三条命令，只产生一个commit，第二个commit修正了第一个commit。
...
--------|-------|-----------------
git reset | Commit-level | Discard commits in private branch or throw away uncommited changes
...
git rm --cached  ignore/ignore.file   # 删除追踪track状态，而不是物理文件
git commit -m 'We really don't want Git to track this anymore!'

````

注意，`gitignore`文件，只能用于Untraked Files，用来没有Git track的键，如果文件add之后，则不能ignore。
...
### Undo
[Reset, Checkout, and Revert](https://www.atlassian.com/git/tutorials/resetting-checking-out-and-reverting/summary)
