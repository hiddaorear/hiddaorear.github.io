...
JavaScript函数的一大特点，函数存在`定义时上下文`，`运行时上下文`，`上下文是可以改变的`。所以就有了`call`,`apply`,`bind`。

四句话总结：

1. 三者都是用来改变函数的this指针的指向；
2. 三者的第一个参数都是this指针说要指向的对象，即函数执行的上下文；
3. 都可以利用后续参数继续传参；
4. bind是返回对应的函数，用于以后调用，当前并不调用；apply，call则是立即调用，apply与call只是传参方式有别而已，apply用来处理数组相关的很方便。

## bind

### 偏函数(Partial Functions)

Partial Functions 也叫 Partial Applications

> Partial application can be described as taking a function that accepts some number of arguments, binding values to one or more of those arguments, and returning a new function that only accepts the remaining, un-bound arguments.

一句话总结，bind可以用来预定参数。

bind的使用：单体模式中，去this指针缓存(常有需要用sef，that，_this等缓存this指针)。可以改变上下文，继续引用对应的this指针。
...
因为JavaScript特有的机制，this指针不是良家妇女，从不从一而终，而是烟花柳巷的水性杨花女子，谁有钱就跟谁。所以在`$('.someClass').on()`中this发生了改变。用bind可以优雅的解决这个问题，this从良。
...
function ReplaceProcessor() {
  ...
  this._dom.brnComplete.on('click', this._onComplete.bind(this))
...
[Javascript通过bind()掌控this](http://m.oschina.net/blog/265415)

[Partial Application in JavaScript](http://benalman.com/news/2012/09/partial-application-in-javascript/)

[Javascript中bind()方法的使用与实现](http://liuwanlin.info/javascriptzhong-bindfang-fa-de-shi-xian/)
