...
人类第一个真正的抽象学科是抽象代数，抽象代数是从伽罗华的群论发展起来的。伽罗华是数学从研究计算，转向研究结构的关键人物。从此，数学从古代转向近现代。在其之前，数学是靠计算解决问题。伽罗华以超凡的洞察力，构建了从数学结构来研究数学问题的群论，这是从具体到抽象的巨大跨越。

希尔伯特在《几何基础》（Grundlagen der Geometrie, 1899）中说：

> 必须可以用”点、线、面“以外的概念（如“桌子、椅子、啤酒杯”）替代几何对象，只要它们满足公理。

希尔伯特是数学的亚历山大大帝。几何的本质是抽象关系，具体的对象不重要。因此，只要抽象关系成立（公理），几何元素可以替换为任何具象。关系重于实体，形式高于具象。

在数学物理中，诺特定理（Noether's Theorem）指出：物理系统的每一个连续对称性，都对应一个守恒的物理量。对称即守恒，而对称即群。

**诺特定理：对称性、守恒量与对称群的对应关系**

| **对称性**               | **守恒量**         | **群**                  | **数学描述**                | **典型系统**              |
|-------------------------|------------------|---------------------------|--------------------------|--------------------------|
| 时间平移对称性           | 能量               | 实数加法群 (ℝ, +)           | 哈密顿量时间不变性          | 封闭力学系统             |
...
量子力学是科幻作品中的常客，刘慈欣先生的《球状闪电》中，很生动描述了量子力学的哥本哈根诠释。量子力学中还有名鼎鼎的物理学四大神兽之一，薛定谔的猫。其他三只是拉普拉斯兽，麦克斯韦妖，芝诺的乌龟。薛定谔的猫让人疑惑的问题之一，是量子力学的量子态的叠加。
...
其中 $\alpha$ 和 $\beta$ 是概率幅，可以是正数、负数、复数。Shor's Algorithm 中量子傅立叶变换的部分，就跟概率幅可以为负数相关。
...
> Hadamard 最不能忘怀的事情，是关于自己当初考试的。以至于年纪大的时候，仍然耿耿于怀，甚至到俄国和 Kolmogorov 都提这件事。就是 Hadamard 做学生的时候，参加数学的会考（相当于数学竞赛吧），得了第二名，第一名后来也是一个数学家，Hadamard 对 Kolmogorov 说："事实证明后来他做得没有我好，其实他一直没有我好。"
...
量子之间可以叠加，还可以纠缠。例如：当 2 个电子纠缠在一起，一个电子自旋向上，另一个电子自旋向下。此时，测量一个电子自旋，若为向上，另一个电子必然会向下，反之亦然。即使两个电子之间隔着一个银河系也是如此。量子纠缠有大名鼎鼎的贝尔不等式实验证明。刘慈欣先生的《三体》中智子，与三体人的通信，设定上是通过量子纠缠实现。

综上所述，量子计算机中的计算是可逆的。量子态可以叠加，也可以纠缠在一起。利用量子的这些特性，可以实现相对于传统计算机的性能，对部分计算问题有指数级的提升。
...
- $\alpha$ 和 $\beta$ 是复数概率幅，满足归一化 $ \|\alpha\|^2 + \|\beta\|^2 = 1 $
...
1982年，诺贝尔物理学奖得主费曼在演讲《用计算机模拟物理》（Simulating Physics with Computers）提出经典计算机无法高效模拟量子系统。12年后数学家彼得·秀尔（Peter Williston Shor）在1994年发现的，Shor's Algorithm ，又称量子质因数分解算法。这个算法很重要，在量子计算机领域独领风骚。

Shor's Algorithm 能在多项式时间内分解大整数（RSA加密的基础），引发量子计算研究的热潮。Shor 因此获得了理论计算机科学成就的最高奖——内万林纳奖（Nevanlinna Prize）。
