...
Shor's Algorithm 影响深远，时至今日，说起量子计算机的进展，常用秀尔算法的实现规模来描述，如支持能把 15 分解为 $5\times3$ 之类的叙述（2001 年IBM的量子计算机）。
...
以 $2^r \mod 15$ 为例子，经过1和2，此时得到如下结果：
  ...
  由数论中的欧拉定理，寄存器 register2 中的值，会以 r 为周期重复出现；为了简化问题的论述，假设 $r\modL = 0$。
